...
give bots id value = take id bots ++ [newBot] ++ drop (id+1) bots
  where newBot = give' (bots !! id) value
...
processInstructions bots [] (instr:uctions) = case finished bots of
  True -> bots
  False -> case processable bots instr of
    True -> processInstructions (processInstruction bots instr) [] uctions
    False -> processInstructions bots [instr] uctions
processInstructions bots queue [] = processInstructions bots [] queue
processInstructions bots queue@(q:ueue) instructions@(instr:uctions) = case finished bots of
  True -> bots
  False -> case processable bots q of
    True  -> processInstructions (processInstruction bots q) ueue instructions
    False -> case processable bots instr of
      True  -> processInstructions (processInstruction bots instr) queue uctions
      False -> processInstructions bots (instr:queue) uctions
...
main = do
  ...
  putStrLn $ show $ filter (\x -> lo x == 17 && hi x == 61) $ processInstructions bots [] (lines input)