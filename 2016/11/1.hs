import Data.List
import Data.Set (Set, empty, fromList, member, toList, unions, singleton)
import qualified Data.Set (delete, map, union, filter)
import Debug.Trace
...
parseLine line = case filteredWords of
  ...
  where filteredWords = filter (/= "compatible") $ words $ map (\x -> if elem x ['.', ',', '-'] then ' ' else x) line

parse input = (map fromList $ map parseLine $ lines input, (empty, 1))
...
oneItemMovedUpOrDown (floors, (_, elevLevel)) = [(map (\x -> Data.Set.delete item x) floors, (fromList [item], newLevel)) | item <- toList currFloor, newLevel <- [elevLevel-1, elevLevel+1], elem newLevel [1..4]]
  where currFloor = floors !! (elevLevel - 1)
-- Two items:
twoItemsMovedUpOrDown (floors, (_, elevLevel)) = [(map (\x -> Data.Set.delete item2 $ Data.Set.delete item1 x) floors, (fromList [item1, item2], newLevel)) | item1 <- toList currFloor, item2 <- toList currFloor, item1 /= item2, newLevel <- [elevLevel-1, elevLevel+1], elem newLevel [1..4]]
  where currFloor = floors !! (elevLevel-1)
...
floorIsLegal floor = allMatched (generators floor) (microchips floor)
  where generators floor = fromList [x | x@(G _) <- toList floor]
        microchips floor = fromList [x | x@(M _) <- toList floor]
        allMatched gs ms = if gs == empty then True else all (== True) $ Data.Set.map (\(M x) -> member (G x) gs) ms
