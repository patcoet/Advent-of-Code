  ...
  if stream[i] == '<':
    for j in range(i+1, len(stream)):
      if stream[j] == '>':
        ...
        garbage_patch_length = len(stream[i:j+1]) - 2
        total_garbage_amount = total_garbage_amount + garbage_patch_length
...
print("Total garbage:", total_garbage_amount)