...
def tick(space):
  ...
  for coord in space.keys():
    ...
    for neighbor_coord in [(x, y, z, w) for x in pm[0] for y in pm[1] for z in pm[2] for w in pm[3] if (x, y, z, w) != coord]:
...
for cycle in range(6):
  # print(f"Active hypercubes at cycle {cycle}: {len([x for x in space.values() if x == '#'])}")
  space = tick(space)
