...
fn main() {
    ...
    let biggest = totals.iter().fold(0, |acc, x| cmp::max(acc, *x));
    let unused_space = total_space - biggest;

    println!(
        "{}",
        totals
            .iter()
            .filter(|x| **x >= needed_space - unused_space)
            .fold(biggest, |acc, x| cmp::min(acc, *x))
    );
}
