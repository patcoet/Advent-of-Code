...
instance Show Monkey where
  show (Monkey num items _ _ _ _ insps) = show (num, items, insps)
...
parseMonkey :: Int -> [String] -> Monkey
parseMonkey factor (l0:l1:l2:l3:l4:l5:[]) = Monkey num items op test ift iff 0
  where
    num = read $ init $ words l0 !! 1
    items = map read $ map (takeWhile isDigit) $ drop 2 $ words l1
    op = \x -> mod ((if words l2 !! 4 == "+" then (+) else (*)) x (if isDigit (head (words l2 !! 5)) then read (words l2 !! 5) else x)) factor
...
main = do
  ...
  let monkeys = [parseMonkey factor $ take 6 $ drop n $ lines input | n <- 0:[7, 14 .. length $ lines input]]
      after10000 = doRounds monkeys 10000
      twoMostActive = take 2 $ sortBy (flip compare) after10000
      insps (Monkey _ _ _ _ _ _ x) = x
      factor = foldr1 (*) [read $ last $ words $ lines input !! n | n <- [3, 10 .. length $ lines input]]
